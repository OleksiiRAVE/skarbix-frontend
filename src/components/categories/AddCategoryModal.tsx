...
import { createCategory } from '@/lib/mock-api/api';
...
export function AddCategoryModal({ open, onOpenChange, onSuccess }: AddCategoryModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success(t('addCategory.toastSuccess'));
      onOpenChange(false);
      onSuccess?.();
    } catch (error) {
      const message = error instanceof Error ? error.message : t('addCategory.toastError');
      toast.error(message);
    } finally {
      setSaving(false);
    }
      ...
      <DialogContent showCloseButton={false} className="sm:max-w-[480px] rounded-[24px] p-0 overflow-hidden border-0 max-w-[calc(100%-16px)] bg-[var(--sk-card)]">
        ...
        <div className="px-6 sm:px-8 space-y-5 max-h-[60vh] overflow-y-auto">
          ...
          <div className="space-y-1.5">
            ...
            <Select
              value={form.kind}
              onValueChange={(value) => setForm((current) => ({ ...current, kind: value as Category['kind'] }))}
            >
              ...
              <SelectContent>
                <SelectItem value="expense">{t('addCategory.kindExpense')}</SelectItem>
                <SelectItem value="income">{t('addCategory.kindIncome')}</SelectItem>
              </SelectContent>
            </Select>
          </div>
        ...
        <div className="p-6 sm:p-8 pt-4 flex gap-3">
          ...
          >
            {saving ? t('addCategory.adding') : t('addCategory.add')}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
